...
import { fetchAppData, update } from '../store'
...
export async function updateMusic() {
  ...
  const { MusicLibrary, MusicPaths, Songs } = appData
  // 删除已经不存在了的文件路径
  ...
  appData.Songs = Array.from(songMap.values())
  appData.MusicPaths = MusicPaths
  update(ElectronStoreKey.APP_DATA, appData)
}
